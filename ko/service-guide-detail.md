...
Dooray!에서는 아웃고잉 웹 훅(Outgoing Webhook)기능을 지원하고 있습니다. Dooray! 프로젝트에 업무나 댓글이 등록될 때, 다른 서비스로 그 내용을 전달하기 위한 방법입니다. 예를 들어 Dooray! 프로젝트에 업무나 댓글이 등록되었을 때 자주 사용하는 슬랙 같은 메신저에서 바로 확인할 수 있는 기능입니다. 아웃고잉 웹 훅은 프로젝트 내용을 어디든 외부로 전송할 수 있는 보안 위험 때문에 프로젝트 관리자만 설정할 수 있습니다. 먼저 관리자가 프로젝트 설정에서 ‘웹 훅 추가’ 버튼을 클릭합니다. 아래의 웹 훅 추가 화면에서 Dooray! 이벤트를 내보낼 웹 훅 URL을 등록합니다. 그리고 어떤 서비스로 이벤트 알림을 전달할지 결정합니다. 현재 Dooray!의 아웃고잉 웹훅은 Dooray !메신저와 슬랙만 지원하고 있습니다. 추가로 다른 서비스 연동을 제공할 예정입니다.
...
![하위추가](http://static.toastoven.net/prod_dooray_project/detail/pjt_detail_external_mail.png)
<center>[그림]외부 이메일 추가</center>  

#### 댓글을 외부 이메일로 발송 

- 댓글로 멤버들끼리 편하게 소통을 하다가, 외부 사용자에게 전달할 내용이 있을 경우 댓글에서 바로 메일을 발송할 수 있습니다. 댓글을 메일로 보낼 때, 보내는 이름을 변경하고 싶다면, 먼저 프로젝트 설정 > 업무 > 메일 연동에서 보내는 이름을 설정합니다. 그리고 댓글에서 메일 발송 시, 보내는 이름을 변경해보세요.
- 댓글을 메일로 발송한 경우, 업무 본문과 동일하게 댓글 편집이 불가합니다.

![하위추가](http://static.toastoven.net/prod_dooray_project/detail/pjt_detail_external_mail02.png)
